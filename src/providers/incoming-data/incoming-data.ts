...
export class IncomingDataProvider {
  ...
  private isValidDarkpayUri(data: string): boolean {
    data = this.sanitizeUri(data);
    return !!this.bwcProvider.getBitcoreDarkpay().URI.isValid(data);
  }
  ...
  private isValidDarkpayAddress(data: string): boolean {
    return !!(
      this.bwcProvider.getBitcoreDarkpay().Address.isValid(data, 'livenet') ||
      this.bwcProvider.getBitcoreDarkpay().Address.isValid(data, 'testnet')
    );
  }
  ...
  private isValidDarkpayPrivateKey(data: string): boolean {
    return !!(data && this.checkDarkpayPrivateKey(data));
  }
  ...
  private handleDarkpayUri(data: string, redirParams?: RedirParams): void {
    this.logger.debug('Incoming-data: DarkPay URI');
    let amountFromRedirParams =
      redirParams && redirParams.amount ? redirParams.amount : '';
    const coin = Coin.D4RK;
    let parsed = this.bwcProvider.getBitcoreDarkpay().URI(data);
    let address = parsed.address ? parsed.address.toString() : '';
    let message = parsed.message;
    let amount = parsed.amount || amountFromRedirParams;
    if (parsed.r) this.goToPayPro(data, coin);
  ...
  private handlePlainDarkpayAddress(
    data: string,
    redirParams?: RedirParams
  ): void {
    this.logger.debug('Incoming-data: DarkPay plain address');
    const coin = Coin.D4RK;
    if (redirParams && redirParams.activePage === 'ScanPage') {
      this.showMenu({
        data,
  ...
  public redir(data: string, redirParams?: RedirParams): boolean {
    ...
    if (this.isValidPayProNonBackwardsCompatible(data)) {
      ...
      // Darkpay  URI
    } else if (this.isValidDarkpayUri(data)) {
      this.handleDarkpayUri(data, redirParams);
      return true;
      ...
      // Plain Address (Darkpay)
    } else if (this.isValidDarkpayAddress(data)) {
      this.handlePlainDarkpayAddress(data, redirParams);
      return true;
      ...
      // Check Darkpay Private Key
    } else if (this.isValidDarkpayPrivateKey(data)) {
      this.handlePrivateKey(data, redirParams, 'darkpay');
      return true;
  ...
  public parseData(data: string): any {
    ...
    if (this.isValidPayProNonBackwardsCompatible(data)) {
      ...
      return {
        data,
        type: 'DarkpayUri',
        title: this.translate.instant('Darkpay URI')
      };
      ...
      return {
        data,
        type: 'DarkpayAddress',
        title: this.translate.instant('Darkpay Address')
      };
  ...
  private checkDarkpayPrivateKey(privateKey: string): boolean {
    // Check if it is a Transaction id to prevent errors
    let isPK: boolean = this.checkRegex(privateKey);
    if (!isPK) return false;
    try {
      this.bwcProvider.getBitcoreDarkpay().PrivateKey(privateKey, 'livenet');
    } catch (err) {
      return false;
    }
    return true;
