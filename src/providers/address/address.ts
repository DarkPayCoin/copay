...
export class AddressProvider {
  private bitcore;
  private bitcoreCash;
  private bitcoreDarkpay;
  private Bitcore;

  constructor(private bwcProvider: BwcProvider) {
    this.bitcore = this.bwcProvider.getBitcore();
    this.bitcoreCash = this.bwcProvider.getBitcoreCash();
    this.bitcoreDarkpay = this.bwcProvider.getBitcoreDarkpay();
    this.Bitcore = {
      btc: {
        lib: this.bitcore,
        translateTo: 'bch'
  ...
  public getCoin(str: string): string {
    ...
    try {
      ...
      try {
        ...
        try {
          new this.Bitcore['darkpay'].lib.Address(address);
          return 'darkpay';
        } catch (e) {
          return null;
        }
  ...
  public getNetwork(str: string): string {
    ...
    try {
      ...
      try {
        ...
        try {
          network = this.bwcProvider.getBitcoreDarkpay().Address(address)
            .network.name;
        } catch (e) {}
      }
    }
  ...
  public extractAddress(str: string): string {
    const extractedAddress = str
      .replace(/^(bitcoincash:|bchtest:|bitcoin:|darkpay:)/i, '')
      .replace(/\?.*/, '');
  ...
  public isValid(str: string): boolean {
    ...
    const URIDarkpay = this.bitcoreDarkpay.URI;
    const AddressDarkpay = this.bitcoreDarkpay.Address;
    ...
    } else if (/^darkpay:/i.test(str)) {
      if (URIDarkpay.isValid(str)) {
        uri = new URIDarkpay(str);
        uriAddress = uri.address.toString();
        if (AddressDarkpay.isValid(uriAddress, 'livenet')) return true;
        if (AddressDarkpay.isValid(uriAddress, 'testnet')) return true;
      }
    }
    ...
    if (AddressDarkpay.isValid(str, 'livenet')) return true;
    if (AddressDarkpay.isValid(str, 'testnet')) return true;
