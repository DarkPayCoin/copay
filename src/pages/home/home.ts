...
export class HomePage {
  ...
  public showReorderDarkpay: boolean;
  ...
  private openEmailDisclaimer() {
    let message = this.translate.instant(
      'By providing your email address, you give explicit consent to Darkpay to use your email address to send you email notifications about payments.'
    );
    ...
    () => {
      ...
      this.walletsBtc = _.filter(this.wallets, (x: any) => {
        if (
          ...
          this.logger.info('Old Darkpay Copay wallet detected, updating...');
          x.coin = 'darkpay';
          x.credentials.coin = 'darkpay';
          this.walletProvider
            .recreate(x)
            .then(() => {
              this.logger.info(
                'Old Darkpay Copay wallet detected, updated successfuly'
              );
            })
            .catch(err => {
              this.logger.error('Error updating old darkpay wallet', err);
            });
          return false;
        }
      ...
      this.walletsDarkpay = _.filter(this.wallets, (x: any) => {
        return x.credentials.coin == 'darkpay';
      });
  ...
  public goToWalletDetails(wallet): void {
    if (this.showReorderBtc || this.showReorderBch || this.showReorderDarkpay)
      return;
    this.events.unsubscribe('finishIncomingDataMenuEvent');
    this.events.unsubscribe('bwsEvent');
  ...
  public reorderDarkpay(): void {
    this.showReorderDarkpay = !this.showReorderDarkpay;
  }
  ...
  public reorderWalletsDarkpay(indexes): void {
    let element = this.walletsDarkpay[indexes.from];
    this.walletsDarkpay.splice(indexes.from, 1);
    this.walletsDarkpay.splice(indexes.to, 0, element);
    _.each(this.walletsDarkpay, (wallet, index: number) => {
      this.profileProvider.setWalletOrder(wallet.id, index);
    });
  }
