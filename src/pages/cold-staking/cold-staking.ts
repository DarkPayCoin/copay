...
export class ColdStakingPage extends WalletTabsChild {
  ...
  constructor(
    ...
    this.darkpayBitcore = this.bwcProvider.getBitcoreDarkpay();
  }
  ...
  public learnMore(): void {
    let url = 'https://darkpaycoin.io/community/staking';
    let optIn = true;
    let title = null;
    let message = this.translate.instant(
      'Learn more about cold staking on the DarkPay wiki.'
    );
    let okText = this.translate.instant('Open DarkPay Wiki');
    ...
    this.popupProvider
      .ionicConfirm(
        ...
        this.translate.instant(
          'This means you will stop receiving interest on your D4RK coins.'
        ),
  ...
  private async balanceTransfer(isZap): Promise<any> {
    try {
      ...
      if (isZap) {
        txp.outputs[0].script = this.darkpayBitcore.Script.fromAddress(
          spend_address,
          stake_address
        ).toString();
      }
